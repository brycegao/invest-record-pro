...
买入交易表单 * * Copyright (c) 2026 brycegao * * Licensed under the MIT License. * See LICENSE file
    ...
    <NDrawerContent title="买入">
      <NForm ref="formRef" :model="formData" :rules="formRules" label-placement="top">
        ...
        <NDescriptions :column="1" bordered size="small">
          <NDescriptionsItem label="总金额（自动计算）">
            {{ formatMoney(totalAmountFen) }}
          </NDescriptionsItem>
          <NDescriptionsItem label="公式">price × quantity</NDescriptionsItem>
        </NDescriptions>
...
import type { Mood, Trade, TradeCreatePayload } from '@/domain/types'
...
const formRules: FormRules = {
  ...
  quantity: { required: true, type: 'number', min: 1, message: '请输入成交数量', trigger: 'blur' },
}
...
async function loadPlanOptions(assetId: number): Promise<void> {
  ...
  try {
    planOptions.value = await getPlanOptions(assetId, 'buy')
  } catch {
